...
//bool cfg_t::misc_t::in_peek_assist()
//{
//	return peek_assist.get() && !misc::retract_peek && misc::has_position;
//}
//
//bool cfg_t::aim_t::headshot_only()
//{
//	var(mp_damage_headshot_only);
//	return hsmode.get() || mp_damage_headshot_only->get_bool();
//}
//
//bool cfg_t::legit_t::legit_enabled()
//{
//	return enabled.get() || !cfg.aim.enabled.get();
//}
//
//void cfg_t::weapon_config_t::update_current_config(C_BaseCombatWeapon *wep)
//{
//	uint32_t gr;
//	const auto wepid = wep->get_weapon_id();
//
//	if (cfgs[wepid])
//		gr = wepid;
//	else
//	{
//		gr = menu::group::general_id;
//
//		for (auto &[gid, weps] : menu::group::grp_to_wep)
//		{
//			if (util::in_array((uint32_t)wepid, weps) && cfgs[gid])
//			{
//				gr = gid;
//				break;
//			}
...
//	}
//
//	is_scout = wepid == WEAPON_SSG08;
//
//	cur.hitchance = hitchance[gr];
//	cur.pointscale = pointscale[gr];
//	cur.mindmg = mindmg[gr];
//	cur.mindmg_override = mindmg_override[gr];
//	cur.mindmg_override_enabled = mindmg_override_enabled[gr];
//	cur.autostop = autostop[gr];
//	cur.autorevolver = autorevolver[gr];
//	cur.scout_jumpshot = scout_jumpshot[gr];
//	cur.autoscope = autoscope[gr];
//	cur.ignore_limbs = ignore_limbs[gr];
//	cur.autostop_mode = autostop_mode[gr];
//	cur.baim = baim[gr];
//	cur.extra_safety = extra_safety[gr];
//	cur.force_safe = force_safe[gr];
//	cur.hitboxes = hitboxes[gr];
//	cur.pointscales = pointscales[gr];
//}
//
//void cfg_t::legit_t::legit_weapon_config_t::update_current_config(C_BaseCombatWeapon *wep)
//{
//	uint32_t gr;
//	const auto wepid = wep->get_weapon_id();
//
...
//	}
//
//	cur.magnet = magnet[gr];
//	cur.triggerbot = triggerbot[gr];
//	cur.rcs = rcs[gr];
//	cur.triggerbot_only_in_scope = triggerbot_only_in_scope[gr];
//	cur.magnet_only_in_scope = magnet_only_in_scope[gr];
//	cur.magnet_smooth = magnet_smooth[gr];
//	cur.magnet_fov = magnet_fov[gr];
//	cur.magnet_speed = magnet_speed[gr];
//	cur.override_factor = override_factor[gr];
//	cur.limit_factor = limit_factor[gr];
//	cur.target_delay = target_delay[gr];
//	cur.reaction_time = reaction_time[gr];
//	cur.rcs_speed = rcs_speed[gr];
//	cur.magnet_hitboxes = magnet_hitboxes[gr];
//}
//
//
//int cfg_t::cur_weapon_config_t::get_mindmg(C_CSPlayer *player, int highest_damage) const
//{
//	const auto minimum_dmg = get_true_mindmg();
//	if (!player)
//		return std::min(100, minimum_dmg);
//
//	const auto additional_damage = std::max(0, minimum_dmg - 100);
//	const auto adjusted_mindmg = std::min(
//		100, std::min(std::min(100, minimum_dmg), std::min(highest_damage, player->get_predicted_health()) +
//			additional_damage));
