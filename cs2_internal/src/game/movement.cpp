...
{
	auto &cmd = game->input->commands[game->input->get_current_command() % 150];

	pressed_jump = cmd.buttons.held & sdk::schema::in_jump;
	onground_tick[game->input->get_current_command() % 150] = game->local_player_controller->get_pawn()->get_flags() & sdk::flags::fl_onground;
	remove_tick[game->input->get_current_command() % 150] = false;
	...
	const auto in_water = game->local_player_controller->get_pawn()->get_water_level() >= 2;
	const auto bad_mt = std::ranges::find(bad_mts, game->local_player_controller->get_pawn()->get_move_type()) != std::end(bad_mts);
	if (bad_mt || in_water)
		return;

	if (!onground_tick[game->input->get_current_command() % 150] || onground_tick[game->input->get_current_command() % 150] && onground_tick[(game->input->
		get_current_command() - 1) % 150] && !remove_tick[(game->input->get_current_command() - 1) % 150])
	{
		cmd.buttons.held &= ~sdk::schema::in_jump;
		cmd.buttons.pressed &= ~sdk::schema::in_jump;
		cmd.buttons.scrolled &= ~sdk::schema::in_jump;
		remove_tick[game->input->get_current_command() % 150] = true;
	}
}
...
void movement_t::strafe()
{
	...
	const auto velocity = game->local_player_controller->get_pawn()->get_vec_abs_velocity();

	if (!game->local_player_controller->get_pawn_is_alive() || game->local_player_controller->get_water_level() >= 2 || game->local_player_controller->get_move_type() ==
		sdk::schema::move_type_t::movetype_noclip || game->local_player_controller->get_move_type() == sdk::schema::move_type_t::movetype_ladder)
		return;
	...
	/*const auto weapon = game->local_player_controller->get_pawn()->get_weapon_services_ptr()->get_h_active_weapon().get();
	const auto js = weapon && (cfg.weapon_config.is_scout && cfg.weapon_config.cur.scout_jumpshot && game->local_player_controller->get_pawn()->get_vec_abs_velocity().length_2d() < 50.f);
	...
	if (game->local_player_controller->get_pawn()->get_flags() & sdk::flags::fl_onground)
		return;
	...
	const bool side_switch = game->input->get_current_command() % 2 == 0;
