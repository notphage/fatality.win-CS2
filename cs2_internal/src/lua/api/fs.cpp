...
namespace lua::api_def::fs
{
	int read(lua_State *l)
	{
		...
		{
			s.error(XOR("read is not available with Allow insecure disabled"));
			return 0;
		}

		if (!s.check_arguments({{ltd::string}}))
		{
			s.error(XOR("usage: fs.read(path)"));
			return 0;
		}
		...
		if (!file.is_open())
		{
			s.error(XOR("failed to open file"));
			return 0;
		}
	...
	int read_stream(lua_State *l)
	{
		...
		if (!cfg.lua.allow_insecure.get())
		{
			s.error(XOR("read_stream is not available with Allow insecure disabled"));
			return 0;
		}

		if (!s.check_arguments({{ltd::string}}))
		{
			s.error(XOR("usage: fs.read_stream(path)"));
			return 0;
		}
	...
	int write(lua_State *l)
	{
		...
		if (!cfg.lua.allow_insecure.get())
		{
			s.error(XOR("write is not available with Allow insecure disabled"));
			return 0;
		}

		if (!s.check_arguments({{ltd::string}, {ltd::string}}))
		{
			s.error(XOR("usage: fs.write(path, data)"));
			return 0;
		}
	...
	int write_stream(lua_State *l)
	{
		...
		if (!cfg.lua.allow_insecure.get())
		{
			s.error(XOR("write_stream is not available with Allow insecure disabled"));
			return 0;
		}

		if (!s.check_arguments({{ltd::string}, {ltd::table}}))
		{
			s.error(XOR("usage: fs.write_stream(path, data)"));
			return 0;
		}
	...
	int remove(lua_State *l)
	{
		...
		if (!cfg.lua.allow_insecure.get())
		{
			s.error(XOR("remove is not available with Allow insecure disabled"));
			return 0;
		}

		if (!s.check_arguments({{ltd::string}}))
		{
			s.error(XOR("usage: fs.remove(path)"));
			return 0;
		}
	...
	int exists(lua_State *l)
	{
		...
		if (!s.check_arguments({{ltd::string}}))
		{
			s.error(XOR("usage: fs.exists(path)"));
			return 0;
		}
	...
	int is_file(lua_State *l)
	{
		...
		if (!s.check_arguments({{ltd::string}}))
		{
			s.error(XOR("usage: fs.is_file(path)"));
			return 0;
		}
	...
	int is_dir(lua_State *l)
	{
		...
		if (!s.check_arguments({{ltd::string}}))
		{
			s.error(XOR("usage: fs.is_dir(path)"));
			return 0;
		}
	...
	int create_dir(lua_State *l)
	{
		...
		if (!cfg.lua.allow_insecure.get())
		{
			s.error(XOR("create_dir is not available with Allow insecure disabled"));
			return 0;
		}

		if (!s.check_arguments({{ltd::string}}))
		{
			s.error(XOR("usage: fs.create_dir(path)"));
			return 0;
		}
