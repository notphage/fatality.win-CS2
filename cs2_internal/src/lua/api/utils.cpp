...
int api_def::utils::find_interface(lua_State *l)
{
	...
	{
		s.error(XOR("find_interface is not available with Allow insecure disabled"));
		return 0;
	}
	...
	const auto res = (int)helpers::get_interface(::utils::fnv1a(s.get_string(1)), ::utils::fnv1a(s.get_string(2)));
...
int api_def::utils::find_pattern(lua_State *l)
{
	...
	if (!cfg.lua.allow_insecure.get())
	{
		s.error(XOR("find_pattern is not available with Allow insecure disabled"));
		return 0;
	}
	...
	try
	{
		const auto result = helpers::find_pattern(helpers::find_module(::utils::fnv1a(s.get_string(1))), s.get_string(2), v);
		...
		case helpers::scan_result::not_found:
			s.error(XOR("pattern: not found"));
			break;
		case helpers::scan_result::too_large_deref:
			s.error(tfm::format(XOR("pattern: too large address to get (max 4 bytes) at %d"), result.error_pos));
			break;
		case helpers::scan_result::too_small_deref:
			s.error(tfm::format(XOR("pattern: too small address to get (min 1 byte) at %d"), result.error_pos));
			break;
		case helpers::scan_result::not_a_power:
			s.error(XOR("pattern: offset is not a power of 2"));
			break;
		case helpers::scan_result::exception:
			s.error(XOR("pattern: exception while reading memory"));
			break;
		case helpers::scan_result::parse_error:
			s.error(tfm::format(XOR("pattern: parse error at %d"), result.error_pos));
			break;
		default:
			break;
		}
	...
	{
		s.error(XOR("pattern: unknown exception"));
	}
...
int api_def::utils::create_timer(lua_State *l, bool run)
{
	...
	s.create_user_object<decltype( obj )>(XOR("utils.timer"), &obj);
...
//	if (!r)
//	{
//		s.error(XOR("usage: world_to_screen(x, y, z): x, y | nil"));
//		return 0;
//	}
//
...
{
	runtime_state s(l);
	const auto r = s.check_arguments({{ltd::string}, {ltd::string}, {ltd::function},});

	if (!r)
	{
		s.error(XOR("usage: utils.http_get(url, headers, callback(response))"));
		return 0;
	}

	if (!cfg.lua.allow_insecure.get())
	{
		s.error(XOR("http_get is not available with Allow insecure disabled"));
		return 0;
	}
	...
	if (!url)
	{
		s.error(XOR("invalid url specified"));
		return 0;
	}
	...
	std::thread(
		...
		{
			const auto inet = InternetOpenA(XOR("fatality"), INTERNET_OPEN_TYPE_PRECONFIG, nullptr, nullptr, 0);
			if (!inet)
				return;
			...
			const auto req = HttpOpenRequestA(
				conn, XOR("GET"), url->path.c_str(), nullptr, nullptr, nullptr, INTERNET_FLAG_KEEP_CONNECTION | (url->is_secure ? INTERNET_FLAG_SECURE : 0), 1);
			if (!req)
			{
...
{
	runtime_state s(l);
	const auto r = s.check_arguments({{ltd::string}, {ltd::string}, {ltd::string}, {ltd::function},});

	if (!r)
	{
		s.error(XOR("usage: utils.http_post(url, headers, body, callback(response))"));
		return 0;
	}

	if (!cfg.lua.allow_insecure.get())
	{
		s.error(XOR("http_post is not available with Allow insecure disabled"));
		return 0;
	}
	...
	std::thread(
		[l, cbk, url, headers, body, me]()
		{
			...
			const auto req = HttpOpenRequestA(
				conn, XOR("POST"), url->path.c_str(), nullptr, nullptr, nullptr, INTERNET_FLAG_KEEP_CONNECTION | (url->is_secure ? INTERNET_FLAG_SECURE : 0), 1);
			if (!req)
			{
				InternetCloseHandle(conn);
				InternetCloseHandle(inet);
...
int api_def::utils::json_encode(lua_State *l)
{
	...
	if (!r)
	{
		s.error(XOR("usage: utils.json_encode(table)"));
		return 0;
	}
...
int api_def::utils::json_decode(lua_State *l)
{
	...
	if (!r)
	{
		s.error(XOR("usage: utils.json_decode(json)"));
		return 0;
	}
	...
	{
		s.error(XOR("invalid json string"));
		return 0;
	}
...
//	if (!r)
//	{
//		s.error(XOR("usage: utils.trace(from, to, skip_ent_index): trace_t"));
//		return 0;
//	}
//
//	if (!interfaces::engine()->IsInGame())
//	{
//		s.error(XOR("attempted to call trace while not in game"));
//		return 0;
//	}
//
...
//	s.create_table();
//	{
//		s.create_user_object<decltype( res.endpos )>(XOR("vec3"), &res.endpos);
...
//		s.create_user_object<decltype( res.plane.normal )>(XOR("vec3"), &res.plane.normal);
...
//	if (!r)
//	{
//		s.error(XOR("usage: utils.trace_bullet(item_definition_index, from, to): damage, trace_t"));
//		return 0;
//	}
//
//	if (!interfaces::engine()->IsInGame())
//	{
//		s.error(XOR("attempted to call trace_bullet while not in game"));
//		return 0;
//	}
//
...
//	{
//		s.error(XOR(
//			"usage: utils.scale_damage(damage, item_definition_index, hit_group, armor, heavy_armor, helmet): damage"));
//		return 0;
//	}
//
...
int api_def::utils::load_file(lua_State *l)
{
	...
	if (!r)
	{
		s.error(XOR("usage: utils.load_file(path): string"));
		return 0;
	}

	char path[MAX_PATH];
	GetCurrentDirectoryA(MAX_PATH, path);
	auto main_dir = std::string(path);
	main_dir = main_dir.substr(0, main_dir.size() - 10);

	main_dir.pop_back(); // get rid of null terminator
	main_dir += xorstr_("\\");
	main_dir += s.get_string(1);

	if (!std::filesystem::is_regular_file(path))
	{
		s.error(XOR("could not open file"));
		return 0;
	}
...
{
	runtime_state s(l);
	const auto r = s.check_arguments({{ltd::string}, {ltd::table, true},});

	if (!r)
	{
		s.error(XOR("usage: utils.print_console(string, color = white)"));
		return 0;
	}

	auto col = evo::ren::color::white();
	if (s.get_stack_top() == 2)
		col = lua::extract_color(s, 2);

	sdk::con_color_msg(col, s.get_string(1));
	return 0;
}
...
//	if (!r)
//	{
//		s.error(XOR("usage: utils.print_dev_console(string)"));
//		return 0;
//	}
//
//	console_log::m_in_print = true;
//	reinterpret_cast<void(__stdcall*)(const Color &, const char *)>(
//		make_offset_multi("engine.dll", sig_print_dev_console))(Color(255, 255, 255, 255), s.get_string(1));
...
int api_def::utils::error_print(lua_State *l)
{
	...
	if (!r)
	{
		s.error(XOR("usage: utils.error_print(string)"));
		return 0;
	}
...
namespace lua::api_def::utils
{
	/*int aes256_encrypt(lua_State *l)
	{
		...
		{
			s.error(XOR("usage: utils.aes256_encrypt(key, data)"));
			return 0;
		}
	...
	int aes256_decrypt(lua_State *l)
	{
		...
		if (!s.check_arguments({{ltd::string}, {ltd::string}}))
		{
			s.error(XOR("usage: utils.aes256_decrypt(key, data)"));
			return 0;
		}
	...
	int base64_encode(lua_State *l)
	{
		...
		{
			s.error(XOR("usage: utils.base64_encode(data)"));
			return 0;
		}
	...
	int base64_decode(lua_State *l)
	{
		...
		if (!s.check_arguments({{ltd::string}}))
		{
			s.error(XOR("usage: utils.base64_decode(data)"));
			return 0;
		}
