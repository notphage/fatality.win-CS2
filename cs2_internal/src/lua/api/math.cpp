...
namespace lua::api_def::math
{
	...
	int vec3_length( lua_State* l )
	{
		...
		if ( !r )
		{
			s.error( XOR( "usage: obj:length()" ) );
			return 0;
		}

		const auto len = s.user_data<sdk::vec3>( 1 ).length();
		s.push( len );
		return 1;
	}
	...
	int vec3_length2d( lua_State* l )
	{
		...
		if ( !r )
		{
			s.error( XOR( "usage: obj:length2d()" ) );
			return 0;
		}
	...
	int vec3_dot( lua_State* l )
	{
		...
		if ( !r )
		{
			s.error( XOR( "usage: obj:dot(vec3 other)" ) );
			return 0;
		}
	...
	int vec3_cross( lua_State* l )
	{
		...
		if ( !r )
		{
			s.error( XOR( "usage: obj:cross(vec3 other)" ) );
			return 0;
		}
	...
	int vec3_normalize( lua_State* l )
	{
		...
		if ( !r )
		{
			s.error( XOR( "usage: obj:normalize()" ) );
			return 0;
		}
	...
	int vec3_add( lua_State* l )
	{
		...
		if ( !r && !r2 )
		{
			s.error( XOR( "usage: obj + vec3/number" ) );
			return 0;
		}
	...
	int vec3_calc_angle( lua_State* l )
	{
		...
		if ( !r )
		{
			s.error( XOR( "usage: obj:calc_angle(vec3 other): vec3" ) );
			return 0;
		}
		...
		s.create_user_object( XOR( "vec3" ), &ret );
		return 1;
	}
	...
	int vec3_dist( lua_State* l )
	{
		...
		if ( !r )
		{
			s.error( XOR( "usage: obj:dist(vec3 other): number" ) );
			return 0;
		}
	...
	int vec3_dist2d( lua_State* l )
	{
		...
		if ( !r )
		{
			s.error( XOR( "usage: obj:dist2d(vec3 other): number" ) );
			return 0;
		}
	...
	int vec3_to2d( lua_State* l )
	{
		...
		if ( !r )
		{
			s.error( XOR( "usage: obj:to2d(): vec3" ) );
			return 0;
		}
	...
	int vec3_new_index( lua_State* l )
	{
		...
		{
			s.error( XOR( "second operand should be a number" ) );
			return 0;
		}
	...
	int vector_angles( lua_State* l )
	{
		...
		if ( !r )
		{
			s.error( XOR( "usage: vector_angles(vec3 forward) vec3" ) );
			return 0;
		}
		...
		s.create_user_object( XOR( "vec3" ), &angles );
		return 1;
	}
	...
	int angle_vectors( lua_State* l )
	{
		...
		if ( !r )
		{
			s.error( XOR( "usage: angle_vectors(vec3 angle) vec3, vec3, vec3" ) );
			return 0;
		}
		...
		s.create_user_object( XOR( "vec3" ), &forward );
		s.create_user_object( XOR( "vec3" ), &right );
		s.create_user_object( XOR( "vec3" ), &up );
		return 3;
	}
	...
	int vec3_unpack( lua_State* l )
	{
		...
		if ( !r )
		{
			s.error( XOR( "usage: obj:unpack() number, number, number" ) );
			return 0;
		}
