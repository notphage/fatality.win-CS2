...
int lua::api_def::timer::run_once(lua_State *l)
{
	...
	if (!s.check_arguments({{ltd::user_data}}))
	{
		s.error(XOR("usage: tmr:run_once()"));
		return 0;
	}
...
int lua::api_def::timer::set_delay(lua_State *l)
{
	...
	{
		s.error(XOR("usage: tmr:set_delay(delay)"));
		return 0;
	}
...
int lua::api_def::timer::is_active(lua_State *l)
{
	...
	if (!s.check_arguments({{ltd::user_data}}))
	{
		s.error(XOR("usage: tmr:is_active()"));
		return 0;
	}
