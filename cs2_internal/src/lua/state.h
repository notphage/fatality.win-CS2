...
namespace lua
{
	...
	public:
		...
		lua_Integer get_integer(int pos);
		...
		template <typename T>
		T &user_data(int pos) { return *(T *)get_user_data(pos); }

		template <typename T>
		T *user_data_ptr(int pos) { return *(T **)get_user_data(pos); }
	...
	{
	public:
		explicit runtime_state(lua_State *s) :
			state(s) { should_destroy_state = false; }

		// arguments
		bool check_arguments(const std::vector<argument_desc> &desc, bool relaxed = false);
		bool check_argument_type(int pos, lua_type_def expected_type) const;
	};
} // namespace lua
