...
namespace sdk
{
	...
	public:
		bool is_enemy()
		{
			const auto local_controller = reinterpret_cast<schema::client::ccsplayer_controller *>(game->local_player_controller());
			return !local_controller || !local_controller->get_h_player_pawn().valid() ? true : get_team_num() != local_controller->get_team_num();
		}

		bool is_alive() { return get_life_state() == static_cast<uint8_t>(schema::life_state_t::life_alive); }
	};
	...
	struct cgame_entity_system
	{
		...
		T *cgame_entity_system::get_entity_by_index(const uint32_t index)
		{
			return reinterpret_cast<T *(*)(void *, int)>(game->client.at(offsets::functions::cgame_entity_system::get_entity_by_index))(this, index);
		}
		...
		void for_each(F fn, const bool local)
		{
			...
			const auto local_controller = game->local_player_controller();
