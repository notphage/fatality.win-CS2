...
namespace sdk::schema
{
	...
	T *chandle<T>::get()
	{
		return !valid() ? nullptr : game->game_entity_system->get_entity_by_index<T>(get_entry_index());
	}
	...
	C *chandle<T>::get_as()
	{
		return !valid() ? nullptr : game->game_entity_system->get_entity_by_index<C>(get_entry_index());
	}
