...
namespace hooks::client
{
	...
	void create_move(sdk::ccsgo_input *input, int slot, bool something, bool sub_tick)
	{
		...
		if (!cmd.base || !game->local_player_controller || !game->local_player_controller->get_pawn())
			return hook_manager.create_move->call(input, slot, something, sub_tick);

		if (input->get_input_msg_frame_slots())
		{
			input->get_input_msg_frame_slots()[0].m_target_head_position = *game->local_player_controller->get_pawn()->as<sdk::cs2_base_entity>()->
		...
		if (input->get_input_msg_frame_slots())
		{
			input->get_input_msg_frame_slots()[0].m_target_head_position =
				*game->local_player_controller->get_pawn()->as<sdk::cs2_base_entity>()->get_computed_abs_origin() + sdk::vector{0, 0, 3};
		}
	...
	{
		lua::api.callback(
			FNV1A("on_frame_stage_notify"), [&](lua::state &s) -> int
			{
				s.push(stage);
				s.push(true);
	...
	void override_view(void *rcx, sdk::cview_setup *view_setup)
	{
		...
		if (run_thirdperon)
		{
			...
			progress = clamp(progress + game->global_vars->frame_time * 6.f, 40.f / cfg.misc.thirdperson_dist.get(), 1.f);
				...
				if (menu::men.finalized)
				{
					...
					if (toggle && scripts)
					{
						...
						scripts->for_each_control(
							...
							{
								const auto sel = c->as<evo::gui::selectable_script>();
